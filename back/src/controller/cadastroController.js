...
router.post('/', async (request, response) => {
    ...
    try{
        if( smp === '' || veiculo === '' || motorista === '' || telefone  === '' || f === '' || entrega === '' || tecn === '' || valor === '' || isca === '' || datainicio === '' || datafinal === '' || destino === '' || status === '' || obs === ''){
            response.status(422).send("Campo em branco")
          }
          else{
            await db.createUser(smp, veiculo, motorista, telefone, f, entrega,tecn,valor,isca,datainicio,datafinal,destino,status,obs)
